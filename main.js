...
const stringsToNumbs = (numbers) => {
    let numArray = numbers.map(element => parseInt(element))
      
    return numArray
}

console.log('strNums array strings to numbers:',stringsToNumbs(strNums))

//   // With the same numbers, find the sum of the even values
const sumEvens = (numbers) =>{
    let evens = numbers.map(element => element % 2 == 0)
    return evens.reduce((a,c) => a + c)
}

console.log('sum of even numbers:',sumEvens(stringsToNumbs(strNums)));

//   // Find the index of the first value when added to it's index = 512 (#ATX!!)
const atxIdx = (numArray) =>{
    return numArray.findIndex((e,i) => e + i == 512)
}

console.log('atxID index:', atxIdx(stringsToNumbs(strNums)))
...
const weatherStates = () => {
    ...
    for(const key of weather){
        stateName.push(key.weather_state_name)
    }
    return stateName
}
